...
- `bun run test`: run Oxfmt/Oxlint checks, typecheck, and the test suite.
- `bun run build`: build to `dist/` using `unbuild`.
- `bun run check:fix`: apply Oxfmt formatting and safe Oxlint fixes.
...
- Use Oxfmt for formatting and Oxlint for linting; avoid stylistic one-off changes.
